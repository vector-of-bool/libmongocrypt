...
if [ "$OS" == "Windows_NT" ]; then
  export BSON_EXTRA_CMAKE_FLAGS="${BSON_EXTRA_CMAKE_FLAGS} -DCMAKE_MSVC_RUNTIME_LIBRARIY=MultiThreadedDebug"
fi
...
if [ "$OS" == "Windows_NT" ]; then
  WINDOWS_CMAKE_FLAGS="-Thost=x64 -A x64 -DCMAKE_MSVC_RUNTIME_LIBRARIY=MultiThreadedDebug"
